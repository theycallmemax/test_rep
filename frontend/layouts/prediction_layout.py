...
from frontend.data.remote_data import fetch_prediction_history, fetch_user_balance
...
def create_merchant_cluster_pair(index):
    return html.Div(
        [
            ...
            html.Button(
                "Remove",
                id={"type": "remove-pair", "index": index},
                n_clicks=0,
...
def prediction_history_table(predictions):
    ...
    for (model_name, timestamp, cost, _), preds in batches.items():
        ...
        batch_info = html.Div(
            [
                html.H5(
                    f"{model_name}, {format_timestamp(timestamp)}, Cost: {abs(cost)}",
                    style=heading5_style,
                ),
...
def estimated_cost(total_cost):
    if total_cost is None:
        return html.Div(
            "Select a model, enter parameters and press the button to get prediction.",
            style=text_style,
        )
...
def prediction_layout(user_session):
    ...
    return html.Div(
        [
            ...
            prediction_form(),
            html.Div(
                prediction_history_table(predictions), id="prediction-history-table"
            ),
        ]
    )

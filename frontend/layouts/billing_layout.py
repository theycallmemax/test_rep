...
def deposit_form():
    return html.Div(
        [
            ...
            html.Button(
                "Deposit", id="deposit-button", n_clicks=0, style=primary_button_style
            ),
        ],
    ...
    data = [
        {
            "id": txn["id"],
            "amount": txn["amount"],
            "timestamp": format_timestamp(txn["timestamp"]),
        }
...
def billing_layout(user_session):
    ...
    return html.Div(
        [
            ...
            deposit_form(),
            html.Div(
                transaction_history_table(transactions), id="transaction-history-table"
            ),
        ]
    )

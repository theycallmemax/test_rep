...
class DepositRequest(BaseModel):
    amount: int = Field(
        ..., gt=0, description="The amount to be deposited. Must be greater than 0."
    )
...
class TransactionInfo(BaseModel):
    ...
    timestamp: datetime = Field(
        default_factory=get_now(), description="Timestamp of the transaction."
    )

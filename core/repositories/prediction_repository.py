...
from core.entities.prediction_batch import PredictionBatch
...
class PredictionRepository(BaseRepository):
    ...
    def get_predictions_reports(self):
        with self.session_factory() as session:
            prediction_reports = (
                session.query(
                    Predictor.name, func.count(Prediction.id).label("total_predictions")
                )
    ...
    def get_prediction_history(self, user_id: int) -> List[PredictionBatch]:
        with self.session_factory() as session:
            prediction_batches = (
                ...
                .options(
                    joinedload(PredictionBatch.predictions),
